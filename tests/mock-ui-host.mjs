...
let revision=4, configured=true, model="mock-model-a", baseUrl="https://api.deepseek.com/v1/";
...
if(n==="deepseek_settings")data={schemaVersion:2,revision,model,baseUrl,credentialConfigured:configured,credentialMask:configured?"••••••••":"",nativeReady:configured&&!!model,message:"ok"};
if(n==="deepseek_models_list")data={models,selectedModel:model,message:"ok"};
if(n==="deepseek_settings_save"){model=a.model;revision++;data={schemaVersion:2,revision,model,baseUrl,credentialConfigured:configured,credentialMask:"••••••••",nativeReady:true,message:"ok"};}
if(n==="deepseek_credential_set"){configured=true;baseUrl=a.baseUrl;revision++;data={schemaVersion:2,revision,model,baseUrl,credentialConfigured:true,credentialMask:"••••••••",nativeReady:!!model,message:"ok"};}
if(n==="deepseek_credential_delete"){configured=false;model="";revision++;data={schemaVersion:2,revision,model,baseUrl,credentialConfigured:false,credentialMask:"",nativeReady:false,message:"ok"};}
if(n==="deepseek_connection_test")data={schemaVersion:2,revision,model,baseUrl,credentialConfigured:configured,credentialMask:configured?"••••••••":"",nativeReady:configured&&!!model,message:"ok"};
